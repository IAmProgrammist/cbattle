...
Field Field::generateRandom() {
    ...
    while (1) {
        ...
        for (int i = 0; i < ONE_SHIP && should_keep_going; i++) {
            ...
            if (!result.validateShips()) should_keep_going = false;;
        }
...
bool Field::validateShips() {
    ...
    for (auto& ship : ships) {
        if (ship.is_horizontal) {
            if (!areInBounds(ship.y)) return false;

            for (int x = ship.x; x < ship.x + ship.length; x++) {
                if (!areInBounds(x)) return false;

                if (test_field[ship.y][x] != 0) return false;
                test_field[ship.y][x] = 2;
                
                if (areInBounds(ship.y - 1) && areInBounds(x)) test_field[ship.y - 1][x] = 1;
                if (areInBounds(ship.y + 1) && areInBounds(x)) test_field[ship.y + 1][x] = 1;
                if (areInBounds(ship.y - 1) && areInBounds(x - 1)) test_field[ship.y - 1][x- 1] = 1;
                if (areInBounds(ship.y - 1) && areInBounds(x + 1)) test_field[ship.y - 1][x + 1] = 1;
                if (areInBounds(ship.y + 1) && areInBounds(x + 1)) test_field[ship.y + 1][x + 1] = 1;
                if (areInBounds(ship.y + 1) && areInBounds(x - 1)) test_field[ship.y + 1][x - 1] = 1;
            }
            
            if (areInBounds(ship.y) && areInBounds(ship.x - 1)) test_field[ship.y][ship.x - 1] = 1;
            if (areInBounds(ship.y) && areInBounds(ship.x + ship.length)) test_field[ship.y][ship.x + ship.length] = 1;
        } else {
            if (!areInBounds(ship.x)) return false;

            for (int y = ship.y; y < ship.y + ship.length; y++) {
                if (!areInBounds(y)) return false;

                if (test_field[y][ship.x] != 0) return false;
                test_field[y][ship.x] = 2;
                
                if (areInBounds(y) && areInBounds(ship.x + 1)) test_field[y][ship.x + 1] = 1;
                if (areInBounds(y) && areInBounds(ship.x - 1)) test_field[y][ship.x - 1] = 1;
                if (areInBounds(y - 1) && areInBounds(ship.x - 1)) test_field[y - 1][ship.x- 1] = 1;
                if (areInBounds(y - 1) && areInBounds(ship.x + 1)) test_field[y - 1][ship.x + 1] = 1;
                if (areInBounds(y + 1) && areInBounds(ship.x + 1)) test_field[y + 1][ship.x + 1] = 1;
                if (areInBounds(y + 1) && areInBounds(ship.x - 1)) test_field[y + 1][ship.x - 1] = 1;
            }
            
            if (areInBounds(ship.y - 1) && areInBounds(ship.x)) test_field[ship.y - 1][ship.x] = 1;
            if (areInBounds(ship.y + ship.length) && areInBounds(ship.x)) test_field[ship.y + ship.length][ship.x] = 1;
        }
    }
