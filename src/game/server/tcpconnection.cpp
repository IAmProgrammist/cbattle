...
TCPServerConnection::TCPServerConnection(GameServer* game, QTcpSocket* conn) : QObject(nullptr), ServerConnection(game), conn(conn)  {
    this->conn = conn;
    connect(conn, &QTcpSocket::readyRead, this, &TCPServerConnection::onReadyRead);
    connect(conn, &QTcpSocket::disconnected, this, &TCPServerConnection::onSurrender);
}
...
void TCPServerConnection::onSurrender() {
