...
void FieldWidget::redraw() {
    ...
    for (int i = 0; i < GAME_SIZE + 1; i++) {
        this->addLine(0, i * tile_size_height, parent->width(), i * tile_size_height, style->getPenGrid());
        this->addLine(i * tile_size_width, 0, i * tile_size_width, parent->height(), style->getPenGrid());
    }
    ...
    for (auto & ship : field.ships) {
        ...
        this->addRect(ship_data.first.x(), ship_data.first.y(),
                      ...
                      ship_data.second.y() - ship_data.first.y(), style->getPenShip(), style->getBrushShip());
    }
    ...
    for (int y = 0; y < GAME_SIZE; y++) {
        for (int x = 0; x < GAME_SIZE; x++) {
            if (field.field[y][x] == FieldElement::EMPTY_CHECKED) {
                auto p = new QGraphicsPixmapItem(style->getCrossImg().scaled(tile_size_width * (1 - style->getPaddingPercent()),
                                                                               tile_size_height * (1 - style->getPaddingPercent())));
                p->setPos((x + style->getPaddingPercent()) * tile_size_width, (y + style->getPaddingPercent()) * tile_size_height);
                this->addItem(p);
            } else if (field.field[y][x] == FieldElement::EXPOLSION_CHECKED) {
                auto p = new QGraphicsPixmapItem(style->getExplosionImg().scaled(tile_size_width * (1 - style->getPaddingPercent()),
                                                                               tile_size_height * (1 - style->getPaddingPercent())));
                p->setPos((x + style->getPaddingPercent()) * tile_size_width, (y + style->getPaddingPercent()) * tile_size_height);
                this->addItem(p);
            }
...
std::pair<QPointF, QPointF> FieldWidget::getShipScenePos(Ship& ship) {
    ...
    double end_x = ((ship.x + (ship.is_horizontal ? ship.length : 1)) - style->getPaddingPercent()) * tile_size_width;
    double beg_y = (ship.y + style->getPaddingPercent()) * tile_size_height;
    double end_y = ((ship.y + (ship.is_horizontal ? 1 : ship.length)) - style->getPaddingPercent()) * tile_size_height;
